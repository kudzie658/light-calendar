...
public:
    Event() : m_title("No event"), m_description("No description"), 
              m_start_ptr(std::make_unique<DateTime>()), 
              m_end_ptr(std::make_unique<DateTime>()) {}
    
    Event(const Event& other) 
        : m_title(other.m_title), m_description(other.m_description),
    ...
    Event(std::string ttl, std::string dsp, std::unique_ptr<DateTime> strt, std::unique_ptr<DateTime> stp) 
        : m_title(ttl), m_description(dsp), 
    ...
    inline DateTime& getEnd() { return *m_end_ptr; }
